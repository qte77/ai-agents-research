...
- `docs/archive/` holds retired/superseded docs — excluded from link-checking, not content candidates
...
The `docs/research/rxiv-agentic-papers.md` index is generated by the `rxiv-paper-eval` pipeline. **Never fire two `rxiv-paper-eval` dispatches on the same calendar day concurrently** — `create-triage-pr` opens its PR from a date-stamped branch (`chore/rxiv-paper-triage-<date>`), so same-day runs resolve to the same branch name, the second push is rejected, and that run is lost. Serialize the dispatches. Tracked upstream: [gha-rxiv-paper-eval#71](https://github.com/qte77/gha-rxiv-paper-eval/issues/71).
...
2. Move the file to `docs/archive/<original-subdir>/`
...
`docs/archive/` is in `lychee.toml` `exclude_path` — archived files do not gate CI.
