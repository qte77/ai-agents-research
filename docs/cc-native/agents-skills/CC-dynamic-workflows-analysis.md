...
**Third-party equivalents**: for deep-research agents outside CC (local-deep-research, GPT-Researcher, OpenAI/Gemini Deep Research, AutoScientists — the last built *on* CC subagents) mapped to this fan-out → cross-check → synthesize pattern, see [research-agents-landscape.md § Mapping to the CC /deep-research Harness](../../non-cc/knowledge-management/research-agents-landscape.md#mapping-to-the-cc-deep-research-harness).
