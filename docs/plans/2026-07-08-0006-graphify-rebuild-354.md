...
(the extraction is token-heavy). Do **not** re-run as a per-doc update — a fine-chunk re-extract is
denser than the uniform base graph and lopsides it (see AGENT_LEARNINGS); a refresh must be a full
uniform rebuild. Tracker: [#374](https://github.com/qte77/ai-agents-research/issues/374).
...
Merged from the 2026-07-08 handoff:

- **Sanity check first:** repo root, clean `main`, and graphify resolves —
  `uv tool run --from graphifyy python -c "import graphify"` (install once: `uv tool install graphifyy`).
- Scope extraction subagents to the graphify extraction-spec only — no skills, no `.claude` edits.
- Run `make check_docs` directly; `make lint` aborts at the lychee flake before markdownlint. Lychee
  may also trip on unrelated pre-existing rot (e.g. `usage.ai` in kiro) — merge past it.
